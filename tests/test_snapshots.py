...
def test_singular_build_uniqueassemblies(snapshot):
    """Testing the API for ability to build csv"""
    f = open("tests/inputs/basicBuildExamples/simple1AssemblyArray.json")
    _data = json.load(f)
    _dataString = json.dumps(_data)
    my_api_response = client.post(
        "/builduniqueassemblies", data={"myAssemblyArrayStr": _dataString}, files={}
    )
    snapshot.assert_match(my_api_response.status_code)
    # TODO: fix snapshot of binary zip response containing 2 csv files
    # snapshot.assert_match(my_api_response.content)
...
def test_singular_build_uniqueassemblies_with_files(snapshot):
    """Testing the API for ability to build csv"""
    f = open("tests/inputs/basicBuildExamples/simple1AssemblyArray.json")
    _data = json.load(f)
    _dataString = json.dumps(_data)
...
def test_viewpartlabels_success_without_files(snapshot):
    """Testing the API for ability to validate builds"""
    f = open("tests/inputs/basicPartExamples/partFromCollection.json")
    _data = json.load(f)
    _dataString = json.dumps(_data)
    my_api_response = client.post(
        "/viewpartlabels", data={"myPart": _dataString}, files={}
    )
    snapshot.assert_match(my_api_response.status_code)
    snapshot.assert_match(my_api_response.json())


def test_viewpartlabels_success_with_files(snapshot):
    """Testing the API for ability to validate builds"""
...
def test_dnaFeaturesViewer_success_without_files(snapshot):
    """Testing the API for ability to validate builds"""
    f = open("tests/inputs/basicPartExamples/partFromCollection.json")
    _data = json.load(f)
    _dataString = json.dumps(_data)
    my_api_response = client.post(
        "/dnafeatureviewer", data={"myPart": _dataString}, files={}
    )
    snapshot.assert_match(my_api_response.status_code)
    snapshot.assert_match(my_api_response.json())


def test_dnafeaturesviewer_success_with_files(snapshot):
    """Testing the API for ability to validate builds"""
    f = open("tests/inputs/basicPartExamples/partFromFile.json")
    _data = json.load(f)
    _dataString = json.dumps(_data)
